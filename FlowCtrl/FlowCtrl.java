...
public class FlowCtrl {
            ...
            try {
                ...
                switch (choice) {
                    ...
                    case 5:
                        ...
                        System.out.println("You're all set, " + loggedInUser.name + "! Wishing you balance and calm today.");
                        loggedInUser = null;
                        return;
                    default:
    ...
    static void viewHealthInsights() {
        ...
        String commonSymptom = getMostCommonSymptom();
        ...
        System.out.println("Symptoms: " + commonSymptom);
    ...
    static String getMostCommonSymptom() {
        Map<String, Integer> symptomCount = new HashMap<String, Integer>();
        for (Cycle c : loggedInUser.cycles) {
            String[] list = c.symptoms.split(",");
            for (String s : list) {
                String trimmed = s.trim().toLowerCase();
                if (symptomCount.containsKey(trimmed)) {
                    symptomCount.put(trimmed, symptomCount.get(trimmed) + 1);
                } else {
                    symptomCount.put(trimmed, 1);
                }
            }
        }
    ...
    static void predictNextCycle() {
        ...
        try {
            ...
            System.out.println("This system provides estimates only. Predictions are not 100% accurate and should not replace professional medical advice. Use it to stay informed and prepared.");
            System.out.println("--------------------------------------------------------------------------------------------------------------------------");
